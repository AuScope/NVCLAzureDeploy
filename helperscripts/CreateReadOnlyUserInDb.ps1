...
<#
.SYNOPSIS
    Ensures a contained user in Azure SQL Database 'NVCL' and adds it to roles 'WEBSERVICE' and 'TSGVIEWER.
...
.DESCRIPTION
    ...
    - Grants membership to WEBSERVICE and TSGVIEWER via ALTER ROLE ... ADD MEMBER.
...
"@
        ...
        Invoke-Sqlcmd -ServerInstance $ServerFqdn -Database $Database -AccessToken $token -Query $tsqlGrantRole1 -ErrorAction Stop
        $tsqlGrantRole2 = @"
DECLARE @u    sysname = N'$uLit';
DECLARE @role sysname = N'$rLit2';
...
END
"@
        Invoke-Sqlcmd -ServerInstance $ServerFqdn -Database $Database -AccessToken $token -Query $tsqlGrantRole2 -ErrorAction Stop
        Write-Host "Ensured contained user '$Username' in database 'NVCL' and added to roles 'WEBSERVICE' and 'TSGVIEWER'." -ForegroundColor Green
    }
    finally {
